...
function unary(kind){
  ...
  switch(kind){
    ...
    case 'sin': result = snapZero(Math.sin(toRadians(x))); break;
    case 'cos': result = snapZero(Math.cos(toRadians(x))); break;
    case 'tan': result = snapZero(Math.tan(toRadians(x))); break;
    default: result = x;
  }
  calcCurrent = trimNum(result);
  calcResetNext = true;
...
function computeLevel(count){
  // progressão uniforme: sobe de nível a cada 5 perguntas respondidas
  return Math.floor(count / 5) + 1;
}

/* Pokémon que vão surgindo andando no cenário, a cada 5 níveis.
   Goomy é o primeiro (nível 2) — troque as imagens/nomes pelos seus. */
const WALKERS = [
  { id:'goomyWalker', threshold:2,  img:'images/goomy.png',     species:'Goomy',     color:'#9C6DC4' },
  { id:'marillWalker', threshold:3,  img:'images/marill.png',  species:'Marill', color:'#7C9FC4' },
  { id:'oshawottWalker', threshold:4, img:'images/Oshawott.png',  species:'Oshawott', color:'#7C9FC4' },
  { id:'politoedWalker', threshold:5, img:'images/politoed.png',  species:'Politoed', color:'#7C9FC4' },
  { id:'psyduckWalker', threshold:6, img:'images/Psyduck.png',  species:'Psyduck', color:'#7C9FC4' },
  { id:'wooperWalker', threshold:7,  img:'images/wooper andando.png',  species:'Wooper', color:'#7C9FC4' }
];

function ensureWalkersExist(){
  ...
  WALKERS.forEach((w, idx) => {
    ...
    if(!el){
      ...
      el.className = 'env-walker';
      el.id = w.id;
      // delay negativo escalona a posição inicial de cada um, pra não andarem "grudados"
      el.style.animationDelay = `-${idx * 2.2}s`;
      const img = document.createElement('img');
      img.src = w.img;
      img.alt = `${w.species} andando`;
      img.dataset.species = w.species;
      img.dataset.color = w.color;
...
function computeEquationLevel(level){
  // a dificuldade das contas continua com no máximo 3 estágios
...
function updateLevelUI(){
  ...
  WALKERS.forEach(w => {
    ...
    if(el) el.classList.toggle('active', currentLevel >= w.threshold);
  });
...
function buildEquation(level){
  const effLevel = computeEquationLevel(level);
  const pools = {
    1: ['+','-'],
    2: ['+','-','×','÷'],
    3: ['+','-','×','÷','^']
  };
  const ops = pools[effLevel] || pools[1];
  const op = ops[randInt(0, ops.length-1)];
  let a,b,c;
  if(op === '+'){
    const range = effLevel === 1 ? 20 : effLevel === 2 ? 60 : 120;
    a=randInt(1,range); b=randInt(1,range); c=a+b;
  } else if(op === '-'){
    const range = effLevel === 1 ? 20 : effLevel === 2 ? 80 : 150;
    a=randInt(5,range); b=randInt(1,a); c=a-b;
  } else if(op === '×'){
    const max = effLevel === 2 ? 12 : 15;
